...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            assert(get_caller_address() == self.get_owner(), 'Not the owner calling');
            self.inventory.entry(product).write(self.get_stock(product) + new_stock);
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            self.inventory.entry(product).write(stock - quantity);
        }
