...
// In this exercise we have a struct Team where a Felt252Dict maps the name of a player to its level
// and keeps track of the number of player.
// Using the methods set and get from the Felt252DictTrait, implement the required functions to
// interact with the team Make me compile and pass the test!
...
impl TeamImpl of TeamTrait {
    ...
    fn level_up(ref self: Team, name: felt252) {
        let (entry, value) = self.level.entry(name);
        self.level = entry.finalize(value + 1);
    }
